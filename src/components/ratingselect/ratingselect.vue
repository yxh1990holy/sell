	...
	export default {
		...
		methods: {
			select(type, event) {
				...
				this.sType = type;
				// console.log("ratingselect.vue-select():sType:"+this.sType);
				// 派发事件通知父组件food.vue selectType的改变,将type值传出
				this.$emit('increment', 'selectType', this.sType);
				// console.log("ratingselect.vue-select():selectType:"+this.selectType);				
			},
			toggleContent(event) {
				if(!event._constructed) {
					return;
				...
				// console.log("ratingselect-toggleContent():oContent:"+this.oContent);
				this.$emit('increment', 'onlyContent', this.oContent);
				// console.log("ratingselect.vue-select():onlyContent:"+this.onlyContent);
			}
		},
		computed: {
			positives() {
