			...
			<div class="food-content">
				...
				<div class="content">
					...
					<div class="cartcontrol-wrapper">
						<!-- 此处@cart-add="addCart"是将cart-add事件传递给cartcontrol.vue子组件调用
						<cartcontrol :food="food" @cart-add="cartAdd($event)"></cartcontrol>-->
					...
					<div class="rating-wrapper">
						<ul v-show="food.ratings && food.ratings.length">
							<li class="rating-item" v-for="rating in food.ratings" v-show="needShow(rating.rateType, rating.text)">
								...
								<p class="text">
									<span :class="{'icon-thumb_up': rating.rateType===0, 'icon-thumb_down': rating
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		methods: {
			...
			// // 点击“+”按钮后的效果
			// cartAdd(event) {
			// 	this.$parent.addCart(event.target);
			// },
			needShow(type, text) {
				// console.log("food.vue-needshow():type+"+type+";text+"+text);
				// console.log("food.vue-needshow():selectType+"+this.selectType+";onlyContent+"+this.onlyContent);
